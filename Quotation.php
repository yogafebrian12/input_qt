	...
	public function saveData(){
		...
		$data=array(
			...
			'jmlGlides'=>$this->input->post('glides'),
  			...
  			'pembahanan_treatment'=>$this->input->post('pembahanan_treatment'),
  			'pembahanan_tenaga_treatment'=>$this->input->post('pembahanan_tenaga_treatment'),
  			'MarginPersen'=>$this->input->post('margin'),
  			'BiayaMargin'=>$this->input->post('margintotal'),
  			'TotalMargin'=>$this->input->post('subtotalmargin')
		);
	...
	public function listQt(){
		...
		foreach($list->result() as $dt){
			...
			if($this->session->userdata('level')=='1'){
				...
				//detailQtKode?kode=
				$btn.=' <a class="btn btn-warning btn-sm" href="'.base_url().'quotation/detailQtKode?kode='.$dql['Kode'].'" title="Detail '.$dql['Kode'].'"><i class="fa fa-eye"></i></a>';
