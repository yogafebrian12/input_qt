	...
	public function listQt(){
		...
		foreach($list->result() as $dt){
			...
			if($this->session->userdata('level')=='1'){
				...
				$btn.=' <a class="btn btn-warning btn-sm" href="'.site_url('detailQtKode').'/'.$dt->Kode.'" title="Detail '.$dt->Kode.'"><i class="fa fa-eye"></i></a>';
			...
			if($dt->Foto!=''){
				$row[]='<div class="text-center"><a href="'.base_url().'assets/photo_items/'.$dt->Foto.'" target="__blank"><img class="img-thumbnail" src="'.base_url().'assets/photo_items/'.$dt->Foto.'" style="width:100px;" /></a></div>';
			}else{
			...
			$row[]='<i class="fa fa-usd"></i>&nbsp;'.number_format($dt->USDPrice,2,',','.');
